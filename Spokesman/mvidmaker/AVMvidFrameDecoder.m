...
// This is the last AVCustomFrame object returned via a call to advanceToFrame

@property (nonatomic, retain) AVCustomFrame *lastFrame;
...
- (void) _freeFrameBuffers
{
  self.currentFrameBuffer = nil;
  self.cgFrameBuffers = nil;
  // Drop AVCustomFrame since it holds on to the image which holds on to a framebuffer
  self.lastFrame = nil;
}
...
- (AVCustomFrame*) advanceToFrame:(NSUInteger)newFrameIndex
{
...
# endif // MV_ENABLE_DELTAS
    ...
    if (isDeltas && actualFrameIndex == 0) {
      ...
      AVCustomFrame *frame = [AVCustomFrame aVFrame];
      NSAssert(frame, @"AVCustomFrame is nil");
...
# endif // TARGET_OS_IPHONE
  ...
  if (!changeFrameData) {
    // When no change from previous frame is found, return a new AVCustomFrame object
    ...
    // Return a CGImage wrapped in a AVCustomFrame. Note that a new AVCustomFrame object is returned
...
- (AVCustomFrame*) duplicateCurrentFrame
{
  ...
  // Return a CGImage wrapped in a AVCustomFrame
