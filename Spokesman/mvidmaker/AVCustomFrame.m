...
+ (AVCustomFrame*) aVFrame
{
  AVCustomFrame *obj = [[AVCustomFrame alloc] init];
...
- (void) makeImageFromFramebuffer
{
  ...
  NSImage *nsImage = [[NSImage alloc] initWithCGImage:imgRef size:size];
...
{
  return [NSString stringWithFormat:@"AVCustomFrame %p (isDuplicate = %d), self.image %p, self.cgFrameBuffer %p, self.cvBufferRef %p",
          self,
          self.isDuplicate,
          self.image,
          self.cgFrameBuffer,
